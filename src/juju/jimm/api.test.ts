  ...
  describe("listRelationshipTuples", () => {
    it("fetches relationship tuple result with supplied params", async () => {
      ...
      const conn = {
        facades: {
          jimM: {
            listRelationshipTuples: vi.fn(() => Promise.resolve(true)),
          },
        },
      } as unknown as Connection;
      const response = await listRelationshipTuples(conn, tuple);
      expect(conn.facades.jimM.listRelationshipTuples).toHaveBeenCalledWith(
        tuple,
      );
      expect(response).toStrictEqual(true);
    });
    ...
    it("handles errors", async () => {
      ...
      const conn = {
        facades: {
          jimM: {
            listRelationshipTuples: vi.fn().mockImplementation(() => {
              throw error;
            }),
          },
        },
      ...
      await expect(
        listRelationshipTuples(conn, relationshipTupleFactory.build()),
      ).rejects.toBe(error);
    });
